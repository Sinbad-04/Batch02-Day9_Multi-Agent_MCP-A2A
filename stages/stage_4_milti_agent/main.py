...
def check_routing(state: LegalState) -> list[Send]:
    """Routing function: dispatch parallel Send objects to specialist nodes based on question keywords."""
...
async def call_privacy_specialist(state: LegalState) -> dict:
    """Privacy specialist sub-agent for GDPR and data protection law."""
    from langgraph.prebuilt import create_react_agent

    print("\n  [Node: call_privacy_specialist] Privacy specialist agent starting...")

    privacy_prompt = (
        "You are a specialist in data privacy law with expertise in GDPR, CCPA, CPRA, "
        "FTC Act Section 5, and data breach notification laws. Analyse the privacy and "
        "data protection aspects of the question. Keep your response under 200 words."
    )
